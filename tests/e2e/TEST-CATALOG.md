...
| 4 | Scene pages expose distinct search intent | Open the Discord and README routes, verify unique H1 and canonical metadata, confirm each converter uses its intended width preset, and check that each page renders a platform-specific fenced-code example plus a link to the sibling workflow. |
...
| 4 | Line art library is substantial in SSR | Fetch `/line-art` without browser JavaScript, verify at least 40 copyable pieces are present, then render the page and confirm the library and paste-safe guidance sections are visible. |
